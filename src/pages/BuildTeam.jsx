...
export default function BuildTeam() {
        ...
        if (getLoginData === null) {
            ...
            (async () => {
                await axios
                    ...
                    .then((response) => {
                        console.log("response.data hhhhh", response.data.user_cricket_player, "f", response.data.user_football_player);
    ...
    const submitForm = async (e) => {
        ...
        const postBody = {
            user_id: storageData.id, tournament_id: id, player_ids: JSON.stringify(state.selections),
        };
                        ...
                        <div className="build-team-area-area basic-temp-main-content-area p-3 p-sm-3 p-md-3 p-lg-5 p-xl-5">
                            <div className="mb-3 row">
                                <div className="col-sm-6 offset-sm-3">
                                    ...
                                    >
                                        ...
                                        {countryList.map((item, index) => (<option key={item.id} value={item.id}>
                                            {item.name}
                                        </option>))}
                                    </select>
                                </div>
                            ...
                            <form onSubmit={submitForm}>
                                <div className="build-player-list-single-item">
                                    {playerList.map((player, index) => (<React.Fragment key={player.id}>
                                        {
